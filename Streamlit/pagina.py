...
from datetime import datetime, timedelta, time # This line is sufficient
...
def load_data(_file_mod=None, data_version=0):
    ...
    try:
        ...
        df['timestamp'] = pd.to_datetime(df['timestamp']).dt.tz_convert('America/Argentina/Buenos_Aires')
...
### CORRECCIÓN 4: DICCIONARIOS COMPLETOS PARA UI DE ALERTAS ###
display_names = {
    'voltaje_a_n': 'Voltaje A-N',
    'corriente_a': 'Corriente A',
    'potencia_activa_total': 'Potencia Activa Total',
    'potencia_reactiva_total': 'Potencia Reactiva Total',
    'potencia_aparente_total': 'Potencia Aparente Total',
    'frequency': 'Frecuencia'
}
default_thresholds_for_display = {
    'voltaje_a_n': {'min': 210.0, 'max': 240.0, 'alert_enabled': True}, # Añadir 'alert_enabled'
    'corriente_a': {'min': 0.0, 'max': 50.0, 'alert_enabled': True},
    'potencia_activa_total': {'min': 0.0, 'max': 12000.0, 'alert_enabled': True},
    'potencia_reactiva_total': {'min': 0.0, 'max': 5000.0, 'alert_enabled': True},
    'potencia_aparente_total': {'min': 0.0, 'max': 13000.0, 'alert_enabled': True},
    'frequency': {'min': 49.8, 'max': 50.2, 'alert_enabled': True}
}

def display_franja_config_form(current_config, franjas_horarias_data):
    current_time = datetime.now().strftime("%H:%M") # This line is correct
    st.subheader("⚙️ Configuración de Umbrales por Franja - Hora actual: {current_time}") # This is the problematic line
    ...
    for variable_key, defaults in default_thresholds_for_display.items():
        ...
        col_min, col_max, col_enabled = st.columns([1, 1, 0.5]) # Añadir una columna para el checkbox
        ...
        initial_enabled = current_umbrales.get(variable_key, {}).get('alert_enabled', defaults['alert_enabled']) # Obtener estado actual o el por defecto
        ...
        with col_enabled:
            st.markdown("<br>", unsafe_allow_html=True) # Espacio para alinear el checkbox
            new_enabled = st.checkbox("Habilitar Alerta", value=initial_enabled, key=f"{selected_franja}_{variable_key}_enabled_checkbox", disabled=not es_admin)
            
        nuevos_umbrales_para_franja[variable_key] = {
            'min': new_min,
            'max': new_max,
            'alert_enabled': new_enabled # Guardar el estado del checkbox
        }
...
def crear_dashboard_alertas(df_alertas):
    ...
    Incluye vistas diarias, por minuto y de distribución.
    """
    st.divider()
    ...
    fig_bar_dia.update_layout(
        title_text='Total de Alertas Agrupadas por Día',
        xaxis_title='Fecha',
    ...
    st.info("💡 Haz clic y arrastra sobre el gráfico para hacer zoom. El control deslizante inferior te permite navegar en el tiempo.")

    df_alertas_minuto = df_alertas.copy()
    df_alertas_minuto['timestamp'] = pd.to_datetime(df_alertas_minuto['timestamp'])
    df_alertas_minuto['timestamp_minuto'] = df_alertas_minuto['timestamp'].dt.floor('min')
    ...
    fig_minuto.update_layout(
        title_text='Detalle de Alertas Minuto a Minuto',
...
def main():
    ...
    if seccion == "📈 Dashboard Principal":
        ...
        if len(date_range) == 2: # Este 'if' envuelve todo el bloque del dashboard
            start, end = date_range
            mask = (df['timestamp'] >= pd.Timestamp(start)) & (df['timestamp'] < pd.Timestamp(end) + timedelta(days=1))
            filtered_df = df[mask]
            
            # Aquí termina el 'if len(date_range) == 2' y comienza su 'else'
            if filtered_df.empty:
                st.warning("⚠️ No hay datos para el rango seleccionado.")
            else: # Este 'else' está anidado dentro del 'if len(date_range) == 2'
                ...
                if not final_cols_to_display:
                    st.info("Por favor, selecciona al menos una columna para mostrar en la tabla.")
                else:
                    st.dataframe(filtered_df[final_cols_to_display], height=400, use_container_width=True)
