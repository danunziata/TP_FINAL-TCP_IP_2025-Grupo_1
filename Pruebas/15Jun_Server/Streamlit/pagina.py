...
def display_franja_config_form(current_config, franjas_horarias_data):
    ...
    if st.session_state.get('roles') == 'admin':
        ...
        if st.button("💾 Guardar Configuración", use_container_width=True):
            # Obtener una copia mutable de las franjas
            franjas_a_guardar = current_config.get('franjas_horarias', {}).copy()
            if selected_franja not in franjas_a_guardar:
                # Si la franja seleccionada no existe en 'franjas_horarias', la creamos
                # con los detalles (inicio_hora, fin_hora) de la franjas_horarias_data original.
                # Esto es importante si el usuario selecciona una franja de 'default_franjas_horarias'
                # y la 'franjas_horarias' principal está vacía.
                franjas_a_guardar[selected_franja] = franjas_horarias_data.get(selected_franja, {
                    'inicio_hora': '00:00', 'fin_hora': '00:00', 'umbrales': {}
                }).copy() # Copia para no modificar el diccionario original directamente
            
            # Actualizar las horas y los umbrales de la franja seleccionada
        ...
        if st.button("💾 Guardar Umbrales (Solo Umbrales)", use_container_width=True):
            ...
            if config_data_non_admin:
                ...
                if selected_franja in franjas_a_guardar_non_admin:
                    # Aseguramos que la estructura para umbrales exista en la franja
...
def wizard_configuracion_franjas(current_config):
    ...
    if st.button("💾 Guardar Configuración Inicial de Franjas", use_container_width=True, type="primary"):
        ...
        # (Esto es una validación simplificada, una robusta es más compleja)
        if dia_inicio >= dia_fin:
            st.error("Error: La hora de inicio del DIA debe ser anterior a la hora de fin del DIA.")
            return
        if noche_inicio >= noche_fin and not (noche_inicio > dia_fin and noche_fin < dia_inicio): # Cruza medianoche
            if not (noche_inicio > dia_fin and noche_fin < dia_inicio):
                st.warning("Advertencia: Las horas de NOCHE parecen cruzar la medianoche. Asegúrate de que las franjas cubran las 24 horas sin solapamiento.")
        
        # Validar que las franjas cubran las 24 horas y no se superpongan
        # Convierte las horas a minutos desde medianoche para fácil comparación
        ...
        # Asumiendo dos franjas, una cubre el gap de la otra.
        # Caso 1: DIA -> NOCHE (Noche cruza medianoche)
        # Ejemplo: DIA 08:00-20:00, NOCHE 20:01-07:59
        cond1 = (dia_fin_min + 1 == noche_inicio_min) and (noche_fin_min + 1 == dia_inicio_min)

        # Caso 2: DIA y NOCHE en el mismo día sin cruzar medianoche, pero cubriendo el día
        # Esto sería menos común si hay solo dos franjas.
        # Por ahora, solo se da una advertencia.
        if not cond1:
            st.warning("Advertencia: Asegúrate de que las franjas horarias cubran las 24 horas del día sin solapamiento para evitar comportamientos inesperados en las alertas.")
        ...
        if save_config(current_config):
            ...
            time.sleep(2)
            st.rerun() # Recargar para que el sistema detecte la nueva configuración
        else:
...
def main():
    ...
    if st.session_state.get('roles') == 'admin':
        ...
        # Si no está definida o está vacía, se asume que necesita el wizard
